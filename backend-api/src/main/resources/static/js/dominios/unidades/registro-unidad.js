document.addEventListener("DOMContentLoaded", function () {
  ...
  function actualizarTorres() {
    var valorTorreSeleccionada = torre.getAttribute('data-selected') || torre.value;
    ...
    if (opcionSeleccionada && opcionSeleccionada.value !== "") {
      ...
      for (var i = 1; i <= numTorres; i++) {
        ...
        opt.value = "Torre " + i;
        opt.textContent = "Torre " + i;
        if (opt.value === valorTorreSeleccionada) {
            opt.selected = true;
        }
  ...
  formulario.addEventListener("submit", function (evento) {
    if (!validarSeleccion(condominio)) { evento.preventDefault(); return; }
    if (!validarTexto(numeroUnidad)) { evento.preventDefault(); return; }
    if (!validarSeleccion(torre)) { evento.preventDefault(); return; }
    if (!validarNumero(piso)) { evento.preventDefault(); return; }
    if (!validarDecimal(area)) { evento.preventDefault(); return; }
  });
});
