document.addEventListener("DOMContentLoaded", function () {
  ...
  if (botonRevertir) {
    botonRevertir.addEventListener("click", function () {
      var confirmacion = confirm("¿Estás seguro de que deseas deshacer los cambios no guardados y restaurar los valores originales?");
      if (confirmacion) {
  ...
  if (botonLimpiarTodo) {
    botonLimpiarTodo.addEventListener("click", function () {
      var confirmacion = confirm('¿Estás seguro de que quieres vaciar todos los campos del propietario y residente? (Los cambios no se guardarán hasta hacer clic en "Guardar")');
      if (confirmacion) {
  ...
  if (botonLimpiarResidente) {
    botonLimpiarResidente.addEventListener("click", function () {
      var confirmacion = confirm("¿Estás seguro de que quieres vaciar los datos del residente?");
      if (confirmacion) {
        limpiarCamposResidente();
      }
    });
